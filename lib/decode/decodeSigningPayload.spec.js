...
function testDecodeSigningPayload(pallet, name) {
    it(`should decode ${pallet}::${name}`, () => {
        const signingPayload = createSigningPayload_1.createSigningPayload(methods[pallet][name](util_1.TEST_METHOD_ARGS[pallet][name], util_1.TEST_BASE_TX_INFO, util_1.TEST_OPTIONS), util_1.TEST_OPTIONS);
        const txInfo = decodeSigningPayload_1.decodeSigningPayload(signingPayload, util_1.TEST_OPTIONS);
